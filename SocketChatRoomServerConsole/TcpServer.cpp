...
void TcpServer::Run()
{
	...
	{
		fd_set copy = master; // Cần tạo copy master vì hàm khi gọi thì hàm select sẽ huỷ luôn fd đó nên bước này quan trọng vl
		...
		for (int i = 0; i < socketCount; i++)
		{
			...
			else // Xử lí tin nhắn yêu cầu của client 
			{
				...
				else
				{
					...
					{
						this->MessageRecieved(this, sock, std::string(buffer, 0, bytesIn));
